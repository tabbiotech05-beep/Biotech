...
// @route   GET api/visits/export-location-report-2026
// @desc    Export Excel report: 2026 months (4 weeks each) vs delegates with governorate & city visit percentages
// @access  Private (Admin only)
router.get('/export-location-report-2026', auth, async (req, res) => {
    try {
        if (req.user.role !== 'admin') {
            return res.status(403).json({ msg: 'Accès non autorisé: administrateur uniquement.' });
        }
        ...
        const visits = await Visit.find({
            start: { $gte: startOfYear, $lte: endOfYear }
        }).select('user delegateName start governorate address targetType').lean();

        // 3. Prepare data structure: delegateId -> month (0..11) -> week (0..3) -> { locations: {}, total: 0 }
        const stats = {};
        const createEmptyMonthWeeks = () => Array.from({ length: 12 }, () =>
            Array.from({ length: 4 }, () => ({ locations: {}, total: 0 }))
        );
        ...
        visits.forEach(v => {
            ...
            // Determine governorate & city
            const gov = (v.governorate || '').trim();
            const addr = (v.address || '').trim();
            let locationLabel = 'Non spécifié';

            if (gov && addr) {
                if (gov.toLowerCase() === addr.toLowerCase()) {
                    locationLabel = gov;
                } else if (addr.toLowerCase().includes(gov.toLowerCase())) {
                    locationLabel = addr;
                } else {
                    locationLabel = `${gov} (${addr})`;
                }
            } else if (gov) {
                locationLabel = gov;
            } else if (addr) {
            ...
            const formattedLoc = locationLabel.charAt(0).toUpperCase() + locationLabel.slice(1);

            stats[userId][month][week].total += 1;
            stats[userId][month][week].locations[formattedLoc] =
                (stats[userId][month][week].locations[formattedLoc] || 0) + 1;
        });
            ...
            for (let m = 0; m < 12; m++) {
                for (let w = 0; w < 4; w++) {
                    ...
                    if (!cellData || cellData.total === 0) {
                        ...
                        weeklyTotals[colIndex] += cellData.total;
                        const entries = Object.entries(cellData.locations);
                        entries.sort((a, b) => b[1] - a[1]);

                        const lines = entries.map(([locName, count]) => {
                            const pct = Math.round((count / cellData.total) * 100);
                            return `${locName}: ${pct}% (${count})`;
                        });
                        lines.push(`Total: ${cellData.total}`);
                        row.push(lines.join('\n'));
                    }
        ...
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, 'Villes & Gouvernorats 2026');

        const buffer = XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });

        res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        res.setHeader('Content-Disposition', 'attachment; filename="Visites_Villes_Gouvernorats_2026.xlsx"');
        return res.send(buffer);
    } catch (err) {
        console.error('[export-location-report-2026] Error:', err);
        res.status(500).json({ msg: 'Erreur lors de la génération du fichier Excel', error: err.message });
    }
});
